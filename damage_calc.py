"""
洛克王国手游 - PVP伤害计算器（竞技场版）
数据来源：wiki.biligame.com/rocom + BV15Zi7BPE9k 视频公式

竞技场属性公式（满级固定，来自视频）：
  洛克系数 L = (种族值 + 个体值/2) / 100
    个体值用 8/9/10 表示，实际值 = 档位×6（即48/54/60）
  生命 = (170L + 70) × 性格系数 + 100
  物攻 = (110L + 10) × 性格系数 + 50
  魔攻 = (110L + 10) × 性格系数 + 50
  物防 = (110L + 10) × 性格系数 + 50
  魔防 = (110L + 10) × 性格系数 + 50
  速度 = (110L + 10) × 性格系数 + 50
  性格系数：加成项=1.2，减成项=0.9，无关项=1.0

PVP伤害公式：
  伤害 = 攻击/防御 × 0.9 × (技能威力×应对倍率 + 威力加成) × 能力等级 × 威力提升buff × 本系补正 × 克制 × 天气 × (1-减伤)
  能力等级 = (1 + 我方攻击提升% + 敌方防御降低%) / (1 + 我方攻击降低% + 敌方防御提升%)
  本系补正：技能属性与攻击方属性相同时×1.25

PVP核心机制（来自wiki攻略/补丁说明）：
  - 速度决定先后手（4/18补丁后迅捷也按速度判断，不再必定先手）
  - 天气：雨天水系+50%、晴天火系+50%
  - 攻防增益：百分比叠加（力量增效、被动层数等）
  - 应对/联防减伤：硬门100%、听桥80%、防御70%
"""
...
DATA_PATH       = Path(__file__).parent / "output" / "精灵完整数据.json"
SKILL_DATA_PATH = Path(__file__).parent / "output" / "技能完整数据.json"
...
def interactive():
    ...
    while True:
        try:
            ...
            atk_up           = float(input("我方攻击提升% (力量增效+30，壮胆+50等) [0]: ").strip() or "0")
            ...
            if atk_trait:
                print(f"  特性：{atk_trait} — {atk_trait_eff}")
            trait_power = float(input("特性威力加成% (如勇敢+40，顺风+50，不生效填0) [0]: ").strip() or "0")
            ...
            def_talent_stats = input_talent_stats("天分属性（3个，空格分隔，如：生命 物防 魔防）: ")
            def_nat          = input("性格 [无]: ").strip() or "无"
            def_up           = float(input("敌方防御提升% (石肤术、硬化等) [0]: ").strip() or "0")
            def_down         = float(input("敌方防御降低% [0]: ").strip() or "0")
            power_add        = float(input("威力加成（固定加值，如极寒领域对冻结+60）[0]: ").strip() or "0")
